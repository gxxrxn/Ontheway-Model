...
def get_train_data(dataset, global_indexing=False):
    unique_sid = list()
...
def get_test_data(unique_sid, unique_uid, dataset, global_indexing=False):
    # unique_sid = list()
    # with open(os.path.join(dataset, 'unique_sid.txt'), 'r') as f:
    #     for line in f:
    #         unique_sid.append(line.strip())
    ...
    data = load_train_data(os.path.join(dataset, 'train.csv'), n_items, n_users, global_indexing=global_indexing)
